...
public class EmbeddedElasticsearchServer {
	...
	public void indexDocument(String indexName, String jsonDocument) {
		IndexResponse response = getClient().prepareIndex(indexName,
			"LiferayDocumentType")
	...
	public void pushMapping(String indexName, String mappingName)
		...
		getClient().prepareIndex(indexName, "LiferayDocumentType");
		...
		PutMappingRequestBuilder pmrb = getClient().admin().indices().
			preparePutMapping(indexName).setType(mappingName);
	...
	public static class Builder {
		...
		public EmbeddedElasticsearchServer build() throws Exception {
			...
			return new EmbeddedElasticsearchServer(_configFileName,
			indexesAndMappings);
		}
	...
	private EmbeddedElasticsearchServer(String configFileName,
	...
	private void deleteDataDirectory() {
		...
		try {
			FileUtils.deleteDirectory(new File(node.settings().get("path.data"))
			);
		}
		catch (IOException e) {
			throw new RuntimeException(
