	...
	<c:choose>
		<c:when test='<%= currentTab.equals("general") %>'>
			...
			<aui:fieldset>
				...
				<aui:input cssClass="lfr-input-text-container" label="notification-sender-name" name="emailFromName" type="text" />
	
				<aui:input cssClass="lfr-input-text-container" label="notification-sender-email-address" name="emailFromAddress" type="text" />
		...
		<c:when test='<%= currentTab.equals("templates") && calendar != null %>'>
			<%
				...
				String bodyTemplate = NotificationUtil.getNotificationTemplateContent(calendar.getCalendarId(), notificationType, templateType, NotificationField.BODY);
				String subjectTemplate = NotificationUtil.getNotificationTemplateContent(calendar.getCalendarId(), notificationType, templateType, NotificationField.SUBJECT);

				String emailFromAddress = NotificationUtil.getNotificationSenderEmailAddress(calendar);
				String emailFromName = NotificationUtil.getNotificationSenderName(calendar);
			%>
			...
			<aui:fieldset>
				...
				<aui:input cssClass="lfr-input-text-container" label="subject" name='<%= subjectParameterName %>' type="text" value="<%= subjectTemplate %>" />

				<aui:field-wrapper label="body">
					<liferay-ui:input-editor editorImpl="ckeditor" />

					<aui:input name='<%= bodyParameterName %>' type="hidden" value="<%= bodyTemplate %>" />
				</aui:field-wrapper>
			</aui:fieldset>
			...
			<aui:script>
				function <portlet:namespace />initEditor() {
					<c:if test='<%= currentTab.equals("templates") %>'>
						return '<%= UnicodeFormatter.toString(bodyTemplate) %>';
					</c:if>
				}
